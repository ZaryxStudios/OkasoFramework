...
public class OkasoBungeeTaskScheduler implements TaskScheduler {

    private final Plugin plugin;
    private final net.md_5.bungee.api.scheduler.TaskScheduler scheduler;

    public OkasoBungeeTaskScheduler(Plugin plugin) {
        this.plugin = plugin;
    ...
    public TaskHandle runAsync(Runnable task) {
        return new OkasoBungeeTaskHandle(scheduler.runAsync(plugin, task));
    }

    @Override
    public TaskHandle runLater(Runnable task, long delay, TimeUnit unit) {
        long millis = unit.toMillis(delay);
        return new OkasoBungeeTaskHandle(scheduler.schedule(plugin, task, millis, TimeUnit.MILLISECONDS));
    }
    ...
    public TaskHandle runTimer(Runnable task, long delay, long interval, TimeUnit unit) {
        ...
        return new OkasoBungeeTaskHandle(scheduler.schedule(plugin, task, delayMillis, intervalMillis, TimeUnit.MILLISECONDS));
    }
